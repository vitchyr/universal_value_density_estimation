...
def run(env_name: str, progressive_noise: bool, reward_factor: float,
    ...
    eval_env = fetch.make_env(env_name, progressive_noise, small_goal,
                              small_goal_size)
    ...
    agent = uvd.UVDTD3(
        functools.partial(fetch.make_env, env_name, progressive_noise,
                          small_goal), device, density_model, q1, q2,
        policy, params)
    fetch.train_fetch(experiment, agent, eval_env, progressive_noise,
                      small_goal)
